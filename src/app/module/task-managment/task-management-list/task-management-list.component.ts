...
export class TaskManagementListComponent implements OnInit {
  ...
  constructor(
    ...
    this.dataService.permission.subscribe((response) => {
      this.permissionObject = response?.permissions?.TASKMANAGEMENT;
    });
  }
  ...
  getObjects() {
    ...
    this.taskManagementService.getTaskManagementList(params).subscribe(
      (response) => {
  ...
  onDelete(data: any): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result && result.is_delete) {
        this.taskManagementService.deleteAddress(data._id).subscribe(
          (res) => {
            if (res.status) {
              this.getObjects();
