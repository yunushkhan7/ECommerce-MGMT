...
export class ProductCategoryAddComponent implements OnInit {
  ...
  // getEditObject() {
  //   this.productService.GetProductcategoryById(this.editId).subscribe((response) => {
  //     if (response) {
  ...
  submitForm() {
    if (this.addForm.valid) {
      this.showLoader = true;
      if (this.isEditing) {
        this.productService.ProductcategoryUpdate(this.editId, this.addForm.value).subscribe(
          (response) => {
            this.showLoader = false;
            if (response) {
              this.router.navigateByUrl('/product-category');
        ...
        this.productService.saveProductcategory({ ...this.addForm.value }).subscribe(
          (response) => {
            this.showLoader = false;
            if (response) {
              this.router.navigateByUrl('/product-category');
