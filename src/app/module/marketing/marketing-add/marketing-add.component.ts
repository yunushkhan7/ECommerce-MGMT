...
@Component({
  selector: 'app-marketing-add',
  templateUrl: './marketing-add.component.html',
  styleUrls: ['./marketing-add.component.scss']
})
export class MarketingAddComponent implements OnInit {
  ...
  getEditObject() {
    this.marketingService.GetMarketingById(this.editId).subscribe((response) => {
      if (response) {
  ...
  submitForm() {
    if (this.addForm.valid) {
      this.showLoader = true;
      if (this.isEditing) {
        this.marketingService.updateMarketing(this.editId,this.addForm.value).subscribe(
          (response) => {
            this.showLoader = false;
            if (response) {
              this.router.navigateByUrl('/marketing');
        ...
        this.marketingService.saveMarketing(this.addForm.value).subscribe(
          (response) => {
            this.showLoader = false;
            if (response) {
              this.router.navigateByUrl('/marketing');
