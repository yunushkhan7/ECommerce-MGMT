...
@Component({
  selector: 'app-procurmenet-analyst-product-add',
  templateUrl: './procurmenet-analyst-product-add.component.html',
  styleUrls: ['./procurmenet-analyst-product-add.component.scss']
})
export class ProcurmenetAnalystProductAddComponent implements OnInit {
      ...
      if (this.isEditing) {
        this.analystService.AnalystUpdate(this.editId,payLoad).subscribe(
          (response) => {
            ...
            if (response) {
              this.router.navigateByUrl('/procurmenet-analyst-product');
            } else {
            }
          },
          (error) => {
